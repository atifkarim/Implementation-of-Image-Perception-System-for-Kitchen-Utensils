...
def read_png(res):
   # try:
    ...
    img = Image.open(StringIO(res).read())
    return np.asarray(img)

def read_npy(res):
    from io import StringIO
    return np.load(StringIO(res).read())
...
for polar_angle in range(polar_angle_start,polar_angle_end,-10):
    ...
    for azimuthal_angle in range(azimuthal_angle_start,azimuthal_angle_end,1):
        ...
        radius=250      #randomly choosen a distance betwen object and camera
        ...
        # the following command came along with unreal cv to set the location and rotaion of the camera
        ...
        #To save image please uncomment the following line
        res = client.request('vget /camera/0/lit F:/unreal_cv_documentation/test_image_save/'+str(pic_num)+'_test_lit.png')
        pic_num+=1
    
    #print("polar_angle",polar_angle,"\z:",z,"\nnew_radius:",new_radius,"\tpitch:",pitch,"\n")
